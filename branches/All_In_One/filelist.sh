...
for i in $(gfind ./data/fonts/ ./data/sounds/ ./data/images/ -maxdepth 1 -type f  \( ! -printf "%f\n" \) | sort -f)
do
	files[count]=$i
	count=$((count+1))
done
...
for i in $(find ./data/fonts/ ./data/sounds/ ./data/images/ -maxdepth 1 -type f  \( ! -printf "%f\n" \) | sort -f)
do
	files[count]=$i
	count=$((count+1))
done
